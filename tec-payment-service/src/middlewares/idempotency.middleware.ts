...
export class RedisIdempotencyStore implements IIdempotencyStore {
  ...
  async set(key: string, entry: CachedResponse): Promise<void> {
    ...
    // Returns null when the key already existed (concurrent duplicate); that is
    // the correct idempotency behaviour – the winner's value is preserved.
  ...
  if (redisUrl) {
    try {
      ...
      const client: IRedisClient = new Redis(redisUrl, {
        ...
        // Auto-connect on first command (no lazyConnect) so the connection is
        // established immediately and the first request doesn't pay the latency.
      });
...
export const idempotency = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  res.json = (body: unknown): Response => {
    ...
    // Fire-and-forget; don't block the response on store writes.
    // Trade-off: under very high concurrency a second identical request that
    // arrives before this write completes may reach the handler a second time.
    // The Redis backend mitigates this via `SET NX` atomicity; the in-memory
    // store accepts the rare duplicate as an acceptable consistency trade-off
    // for avoiding response-path blocking.
