...
export class ProxyService {
  ...
  private readonly services: Record<string, Options & { aliases?: string[] }> = {
    assets: {
      ...
      pathRewrite: { '^/api/assets': '/api/assets' }, // ← صح
    },
    auth: {
    ...
    app.use(
      ...
      createProxyMiddleware({
        ...
        onProxyReq: (proxyReq, req: Request) => {
          ...
          this.logger.debug(`[${routeKey}] ${req.method} ${req.url} → ${target}`);
        },
        onProxyRes: (_proxyRes, _req: Request) => {
          this.logger.log(`[${routeKey} Proxy] Response received with status: ${_proxyRes.statusCode}`);
        },
